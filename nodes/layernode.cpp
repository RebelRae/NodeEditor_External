...
LayerNode::LayerNode(QGraphicsScene *scene, qreal x, qreal y) : EditorNode(scene, x, y) {
    ...
    inputs.push_back(outputIO);

    refreshButton = new QPushButton();
    refreshButton->setText("refresh");

    QGraphicsProxyWidget *rfbProxy = new QGraphicsProxyWidget(nodeItem);
    ...
    QGraphicsProxyWidget *spinProxy = new QGraphicsProxyWidget(nodeItem);
...
void LayerNode::selectLayer(int index) {
    NodeScene *scene = dynamic_cast<NodeScene*>(nodeItem->scene()); // TODO : Check cleanup
